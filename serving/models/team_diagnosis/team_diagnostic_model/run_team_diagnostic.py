...
STAGED_TEAM_STATS = _cfg_path(
    "STAGED_TEAM_STATS", HERE / "lake" / "staged" / "teams" / "team_statistics.parquet"
)
RAW_TEAM_STATS = _cfg_path(
    "RAW_TEAM_STATS", HERE / "lake" / "raw" / "team_stats" / "nfl-team-statistics.csv"
)
STAGED_CONTRACTS = _cfg_path(
    "STAGED_CONTRACTS", HERE / "lake" / "staged" / "players" / "contracts.parquet"
)
...
def _api_team_stats_per_team(base_url: str) -> pd.DataFrame:
    ...
    for abbr in teams:
        ...
        if r.ok:
            ...
            logger.warning(
                "Could not fetch stats for %s (HTTP %s)", abbr, r.status_code
            )
...
def _local_team_stats(
    ...
    raise FileNotFoundError(
        ...
        + (
            ...
            if fallbacks
            else "  No --data-repo supplied and no staged Parquet found in this repo."
        )
    )
...
def _local_contracts(data_repo: Path | None = None) -> pd.DataFrame | None:
    ...
    if data_repo:
        candidates.append(
            data_repo / "lake" / "staged" / "players" / "contracts.parquet"
        )
...
def load_team_stats(
    ...
    if use_api and api_url:
        try:
            ...
            logger.warning("API unreachable (%s) — falling back to local files.", exc)

    return _local_team_stats(data_repo, team_stats_file)


def load_contracts(
    api_url: str | None, use_api: bool = True, data_repo: Path | None = None
) -> pd.DataFrame | None:
    """
...
def main() -> None:
    ...
    parser.add_argument(
        "--season", type=int, default=None, help="Season to summarize (e.g. 2022)"
    )
    parser.add_argument(
        "--team",
        type=str,
    ...
    parser.add_argument(
        "--no-api", action="store_true", help="Skip API and load from local files only"
    )
    ...
    team_stats = load_team_stats(
        api_url, use_api=use_api, data_repo=data_repo, team_stats_file=team_stats_file
    )
    contracts = (
        load_contracts(api_url, use_api=use_api, data_repo=data_repo)
        if args.contracts
        else None
    )
    ...
    print(
        model.top_units(results, "pass_offense", season=season, n=5).to_string(
            index=False
        )
    )
    ...
    print(
        model.top_units(
            results, "pass_defense", season=season, n=5, bottom=True
        ).to_string(index=False)
    )

    print_section(f"Most Over-Performing Teams — {season}  (actual wins > expected)")
    print(
        model.top_units(results, "win_delta", season=season, n=5).to_string(index=False)
    )

    print_section(f"Most Under-Performing Teams — {season}  (actual wins < expected)")
    print(
        model.top_units(
            results, "win_delta", season=season, n=5, bottom=True
        ).to_string(index=False)
    )
