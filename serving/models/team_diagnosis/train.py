...
def train(
    ...
    baseline_rmse = round(
        float(((holdout_full["wins"] - baseline_wins) ** 2).mean() ** 0.5),
        4,
    )
    ...
    train_seasons_range = sorted(
        df_train.index.get_level_values("season").unique().tolist()
    )
    holdout_seasons_range = sorted(
        df_holdout.index.get_level_values("season").unique().tolist()
    )
    ...
    wrapper._metadata.update(
        {
            ...
            "holdout_season_range": [
                holdout_seasons_range[0],
                holdout_seasons_range[-1],
            ],
