...
def _compute_unit_zscores(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for feature_col, direction in feature_directions.items():
        if feature_col in out.columns:
            out[f"{feature_col}_z"] = (
                _zscore_within_season(out, feature_col) * direction
            )
...
def _extract_unit_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ── Pass offense ──────────────────────────────────────────────────────
    out["_feat_po_epa"] = out.get(
        "offense_ave_epa_pass", np.nan
    )  # AVERAGE EPA PER PASS PLAY (EFFICIENCY)
    out["_feat_po_sr"] = out.get(
        "offense_success_rate_pass", np.nan
    )  # SUCCESS RATE ON PASS PLAYS (CONSISTENCY)
    out["_feat_po_ypa"] = out.get(
        "offense_ave_yards_gained_pass", np.nan
    )  # YARDS PER ATTEMPT (VOLUME/EXPLOSIVENESS)
    out["_feat_po_wpa"] = out.get(
        "offense_ave_wpa_pass", np.nan
    )  # WIN PROBABILITY ADDED PER PASS (CLUTCH VALUE)
    ...
    if "offense_n_interceptions" in out.columns:
        ...
        out["_feat_po_int_rate"] = out["offense_n_interceptions"] / n_pass.replace(
            0, np.nan
        )
    else:
        out["_feat_po_int_rate"] = (
            np.nan
        )  # INTERCEPTION DIVIDED BY PASS PER PLAY PENALTY METRIC

    # ── Run offense ───────────────────────────────────────────────────────
    out["_feat_ro_epa"] = out.get(
        "offense_ave_epa_run", np.nan
    )  # AVERAGE EPA PER RUN PLAY (EFFICIENCY)
    out["_feat_ro_sr"] = out.get(
        "offense_success_rate_run", np.nan
    )  # SUCCESS RATE ON RUN PLAYS (CONSISTENCY)
    out["_feat_ro_ypc"] = out.get(
        "offense_ave_yards_gained_run", np.nan
    )  # YARDS PER ATTEMPT (VOLUME/EXPLOSIVENESS)
    out["_feat_ro_wpa"] = out.get(
        "offense_ave_wpa_run", np.nan
    )  # WIN PROBABILITY ADDED PER RUN (CLUTCH VALUE)

    if "offense_n_fumbles_lost_run" in out.columns:
        n_run = out.get("offense_n_plays_run", pd.Series(np.nan, index=out.index))
        out["_feat_ro_fum_rate"] = out["offense_n_fumbles_lost_run"] / n_run.replace(
            0, np.nan
        )
    else:
    ...
    if "defense_n_interceptions" in out.columns:
        ...
        out["_feat_pd_int_rate"] = out["defense_n_interceptions"] / n_pass_d.replace(
            0, np.nan
        )
    else:
...
class TeamDiagnosticModel:
    ...
    def fit(self, team_stats_df: pd.DataFrame) -> "TeamDiagnosticModel":
        ...
        if len(X) < 30:
            logger.warning(
                "Only %d complete rows for regression — model may be unreliable.",
                len(X),
            )
    ...
    def score_teams(
        ...
        df["team_efficiency_z"] = _safe_composite(
            df["offense_composite_z"],
            df["defense_composite_z"],
        ...
        df.sort_values(
            ["season", "team_efficiency_z"], ascending=[True, False], inplace=True
        )
    ...
    def summary(
        self, results_df: pd.DataFrame, season: Optional[int] = None
    ) -> pd.DataFrame:
        """
    ...
    def top_units(
        ...
        return df[
            [
                ...
                "team",
                z_col,
                f"{unit}_rank" if unit != "win_delta" else "overperformance_rank",
            ]
    ...
    def load(cls, path: str | Path) -> "TeamDiagnosticModel":
        ...
        if not isinstance(obj, cls):
            raise TypeError(
                f"Loaded object is {type(obj)}, expected TeamDiagnosticModel."
            )
    ...
    def _append_cap_roi(
        ...
        off_epa_total = df.get(
            "offense_total_epa_pass", pd.Series(0, index=df.index)
        ).fillna(0) + df.get(
            "offense_total_epa_run", pd.Series(0, index=df.index)
        ).fillna(
            0
        )
